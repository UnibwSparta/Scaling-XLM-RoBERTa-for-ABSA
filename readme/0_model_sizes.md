...
- AdamW optimizer.
- [DDP](https://pytorch.org/tutorials/beginner/ddp_series_theory.html) and [FSDP](https://pytorch.org/tutorials/intermediate/FSDP_tutorial.html) for XL and XXL models.
- [CPU offloading](https://www.deepspeed.ai/tutorials/zero-offload/) when training with FSDP, which saves some GPU memory.
- We use PyTorch 2.0, but do not [compile the models](https://pytorch.org/tutorials/intermediate/torch_compile_tutorial.html). Using compiled models can improve training time [up to 30%](https://huggingface.co/docs/transformers/main/perf_torch_compile#benefits-of-torchcompile).
...
- For pre-training, we use an unlabeled dataset with one million tweets. Again, we change the parameters of all layers.
...
The following table shows the training settings, model requirements, and test resutls in terms of training times for unsupervised pre-training on a dataset of one million tweets for one epoch. For the Base, Large, and XL models we run multiple experiments with different numbers of GPUs and batch sizes, trying to keep the global batch size similar as much as possible on our hardware. We used the same hardware for all but one experiment. For the pre-training of the XXL model, we employ NVIDIA H100 hardware with more GPUs and memory.
