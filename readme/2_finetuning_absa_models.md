...
In comparison, a (parameter efficient) fine-tuned version of [GPT-3.5-Turbo achieved a macro F1 score of only over 83%](https://arxiv.org/pdf/2310.18025) in 2024. GPT-3.5, being an autoregressive model, is 17 times larger than the biggest BERT-like model listed in the tables above, yet it still falls short in classification performance. An even worse performance is observed with another GPT-like model, named [Llama-2-7b](https://huggingface.co/meta-llama/Llama-2-7b-hf), according to [Wang et al. (2024)](https://www.spiedigitallibrary.org/conference-proceedings-of-spie/13181/1318187/Fine-tuning-llama-2-and-few-shot-learning-for-ABSA/10.1117/12.3030995.short). We experimented with the successor model [LLama-3.1-8b](https://huggingface.co/meta-llama/Meta-Llama-3.1-8B) of similar size but could not achieve macro F1 scores over 72%. For the interested reader, there is [a complete code example on how we fine-tuned it](https://github.com/UnibwSparta/Scaling-XLM-RoBERTa-for-ABSA/blob/main/finetune_absa_llama_like.py).
